...
def single_son_testcase(testcase, son_testcase, path, is_bulk):
    try:
        ...
        for son_testcase in list(son_testcase):
            single_testcase2(son_testcase, son_yaml_path, is_son_bulk, son_testcase.parent_id, '')
        # config
        name = testcase.name
        yaml = {}
        yaml['test'] = {}
...
def single_testcase(testcase, path, is_bulk, has_parent, testcases_name):
    try:
        ...
        if has_son_testcase:
            ...
            son_yaml_path = os.path.join(path, son_path + '.yml')
            for son_testcase in list(son_testcase):
                single_testcase(son_testcase, son_yaml_path, is_son_bulk, son_testcase.parent_id, '')
        ...
        if not has_son_testcase:
            ...
            if not path_match:
                ...
                testcase_yml_path = os.path.join(path, testcase.name, '{}.yml'.format(testcase.name))
            else:
...
def single_testcase2(testcase, path, is_bulk, has_parent, testcases_name):
    try:
        testcase_id = testcase.id
        son_testcase = TestCase.objects.filter(parent=testcase_id)
        has_son_testcase = len(list(son_testcase)) > 0
...
def run_testcases(testcases_id):
    ...
    for testcase in testcases_list:
        ...
        has_parent = testcase.parent_id
        single_testcase2(testcase, path, is_bulk, has_parent, testcases_name)
